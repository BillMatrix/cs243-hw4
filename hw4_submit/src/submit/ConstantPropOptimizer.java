...
public class ConstantPropOptimizer implements Flow.Analysis {
    ...
    public void postprocess (ControlFlowGraph cfg) {
       ...
       while (qit.hasNext()) {
         ...
         } else if (operator instanceof Operator.Unary) {
           Operator.Unary op = (Operator.Unary) operator;
           if (op.getSrc(q) instanceof RegisterOperand) {
             String operandName = ((RegisterOperand) op.getSrc(q)).getRegister().toString();
             SingleCP cp = in[qid].get(operandName);
             if (cp.isConst()) {
               IConstOperand constOp = new IConstOperand(cp.getConst());
               op.setSrc(q, constOp);
         ...
         } else if (operator instanceof Operator.NullCheck) {
           Operator.NullCheck op = (Operator.NullCheck) operator;
           if (op.getSrc(q) instanceof RegisterOperand) {
             String operandName = ((RegisterOperand) op.getSrc(q)).getRegister().toString();
             SingleCP cp = in[qid].get(operandName);
             if (cp.isConst()) {
         ...
         } else if (operator instanceof Operator.ZeroCheck) {
           Operator.ZeroCheck op = (Operator.ZeroCheck) operator;
           if (op.getSrc(q) instanceof RegisterOperand) {
             String operandName = ((RegisterOperand) op.getSrc(q)).getRegister().toString();
             SingleCP cp = in[qid].get(operandName);
             if (cp.isConst()) {
         ...
         } else if (operator instanceof Operator.Move) {
           Operator.Move op = (Operator.Move) operator;
           if (op.getSrc(q) instanceof RegisterOperand) {
             String operandName = ((RegisterOperand) op.getSrc(q)).getRegister().toString();
             SingleCP cp = in[qid].get(operandName);
             if (cp.isConst()) {
